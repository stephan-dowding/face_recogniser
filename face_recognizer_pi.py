...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    for (x, y, w, h) in faces:
        ...
        if i_conf > 255:
            i_conf = 255
        cv2.rectangle(image, (x, y), (x+w, y+h), (0, 255-i_conf, i_conf), 2)
        cv2.putText(image, "LBPH {}".format(nbr_predicted), (x+5, y+20), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 255, 0), 2)
        cv2.putText(image, "{}".format(confidence), (x+5, y+35), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 0, 255), 2)

        nbr_predicted, confidence = fisher_recogniser.predict(resized)
        cv2.putText(image, "Fisher {}".format(nbr_predicted), (x+5, y+60), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 255, 0), 2)
        cv2.putText(image, "{}".format(confidence), (x+5, y+75), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 0, 255), 2)

        nbr_predicted, confidence = eigen_recogniser.predict(resized)
        cv2.putText(image, "Eigen {}".format(nbr_predicted), (x+5, y+100), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 255, 0), 2)
        cv2.putText(image, "{}".format(confidence), (x+5, y+115), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 0, 255), 2)
    ...
    #clear stream so we don't fall behind
